...
% DIAGNOSTIC MODE
diagnostic_mode = true;   % set true to see per-rep signal stats
...
target_fs_display= 500; % Hz — display/plot only, NOT used for detection

% Collision Detection  (after downsampling)
% Adaptive threshold = median + sensitivity * MAD
%   higher sensitivity -> fewer detections (more selective)
%   lower  sensitivity -> more  detections (more permissive)
accel_sensitivity = 6;
force_sensitivity = 5;
min_distance_sec  = 0.20;   % min seconds between successive detections
merge_window_sec  = 0.20;   % cross-sensor events within this -> one collision
...
%   If quadprog is missing cvxEDA will fall back to PATH A automatically.
...
scr_latency_window = 5.0;   % seconds post-collision to search for SCR peak
%scr_threshold      = 0.05;  % min amplitude to flag SCR (Ohm if raw; z-µS if cvx)
scl_window_start   = 2.0;   % tonic window start (s after collision)
scl_window_end     = 8.0;   % tonic window end   (s after collision)
%scl_threshold      = 0.05;  % min change to flag SCL shift
baseline_before    = 2.0;   % seconds of pre-collision baseline
scr_sensitivity = 2.5;   % n × MAD above noise floor — higher = more selective
scl_sensitivity = 2.0;
% Plotting
% save_figures = true  -> saves one PNG per repetition to rep folder
save_figures = false;
...
for rep_idx = 1:length(all_reps)
    ...
    % Convert absolute event timestamps to same relative timeline as sensors
    if ~isnan(t_trial_start), t_trial_start = t_trial_start - t0_sync; end
    if ~isnan(t_trial_end),   t_trial_end   = t_trial_end   - t0_sync; end

    % Print trial times so you can verify they fall within the session
    if ~isnan(t_trial_start)
        fprintf('  Trial: START=%.2fs  END=%.2fs  (session 0-%.1fs)\n', ...
            t_trial_start, t_trial_end, range(gsr.t));
    else
        fprintf('  Trial events: not found in event.csv\n');
    end

    fprintf('  Data loaded:  GSR %.1fs (%d smp)  Accel %.1fs (%d smp)\n', ...
        range(gsr.t), height(gsr), range(accel.t), height(accel));

    % STEP 1 — Magnitudes + downsample
    
    fs_native = 1 / median(diff(accel.t));
    ds_factor = max(1, round(fs_native / target_fs));
    fs_ds     = fs_native / ds_factor;

    mag_accel_native = max(sqrt(accel.xR.^2 + accel.yR.^2 + accel.zR.^2), ...
        sqrt(accel.xL.^2 + accel.yL.^2 + accel.zL.^2));

    force_mag_native = sqrt(force.fx.^2 + force.fy.^2 + force.fz.^2 + ...
                            force.fw.^2 + force.fq.^2 + force.fr.^2);

    % [mag_accel_ds, t_ds] = downsample_block(mag_accel_native, accel.t, ds_factor);
    % [force_mag_ds,  ~  ] = downsample_block(force_mag_native,  accel.t, ds_factor);
    ...
    % fprintf('  Collision detection (%.0fHz, ds=%d)...\n', fs_ds, ds_factor);
    % min_dist_smp = round(min_distance_sec * fs_ds);
    % 
    ...
    % end
    % force_events = detect_peaks(drv_f, t_ds, thresh_f, min_dist_smp);  %whythis costum function and not findpeak ? 
    % 
    % % Tag each event with its source before merging: 1=accel, 2=force, 3=both
    ...
    % Display-only downsampling comes AFTER detection:
    ds_factor = max(1, round(fs_native / target_fs_display));
    [mag_accel_ds, t_ds] = antialias_downsample(mag_accel_native, accel.t, fs_native, target_fs_display, 4, ds_factor);
    [force_mag_ds,   ~  ] = antialias_downsample(force_mag_native, accel.t, fs_native, target_fs_display, 4, ds_factor);
    ...
    cvx_ok  = false;   % will be set true if cvxEDA succeeds
    ...
    if use_cvxEDA
        ...
        % Try with 'quadprog' first (Optimization Toolbox), then fall back to 'sedumi' or no-solver call which uses cvxEDA's default.
    ...
    % if cvx_ok
    %     % PATH B: use cvxEDA SCR / SCL components
    %     gsr_responses = analyze_gsr_cvxEDA( ...
    %         gsr.t, p_cvx, t_cvx, all_collisions, ...
    %         baseline_before, scr_latency_window, scr_threshold, ...
    %         scl_window_start, scl_window_end, scl_threshold);
    % else
    %     % PATH A: raw skin resistance
    ...
    if save_figures
        saveas(figure(gcf), fullfile(rep_path, [rep_name '_overview.png']));
    end
...
function responses = analyze_gsr_cvxEDA(t_gsr, scr, scl, collision_times, ...
    ...
    for i = 1:length(collision_times)
        ...
        baseline_scr          = mean(scr(bl_mask));
        baseline_scl          = mean(scl(bl_mask));
        responses(i).baseline = baseline_scl;

        % SCR: positive peak in phasic component within scr_window
        ...
        if any(scr_mask)
            ...
            [pk, pi] = max(seg);
            %amp = pk - baseline_scr;
            if pk > scr_thresh %amp > scr_thresh
        ...
        % SCL: sustained mean change in tonic window
...
function responses = analyze_gsr_raw(t_gsr, gsr_raw, collision_times, ...
    ...
    for i = 1:length(collision_times)
        ...
        % SCR: drop in resistance (conductance rise) within scr_window
        scr_mask = (t_gsr >= t0) & (t_gsr < t0 + scr_window);
        if any(scr_mask)
        ...
        % SCL: sustained resistance change in tonic window
        scl_mask = (t_gsr >= t0 + scl_start) & (t_gsr < t0 + scl_end);
        if sum(scl_mask) > 3
...
function plot_overview(gsr, mag_accel_ds, force_mag_ds, t_ds, collision_times, collision_source, ...
    ...
    yl = ylim;
    for i = 1:length(collision_times)
        % Line style = sensor source: solid=accel, dashed=force, dotted=both
        ...
        % Colour = GSR response: red=SCR+SCL, magenta=SCR, cyan=SCL, grey=none
        r = gsr_responses(i);
    ...
    if has_cvx
        ...
        title('cvxEDA\_native: Tonic SCL (blue) + Phasic SCR (red)'); grid on;
    end
...
function plot_collision_panel(ax, gsr, gsr_col, gsr_label, t_col, src, resp, ...
    ...
    xline(ax, 3.0,              ':',  'Color',[0 0.6 0.7],  'LineWidth',1,'HandleVisibility','off');
    ...
    % Trial markers — only drawn if they fall within this panel's time window
    if ~isnan(t_trial_start)
...
function [t_start, t_end] = load_trial_events(filepath)
    ...
    try
        ...
        fprintf('  WARNING: Could not parse event.csv: %s\n', err.message);
    end
end
...
function [gsr, accel, force, t0] = synchronize_time(gsr, accel, force)
    all_times = [];
    if ~isempty(gsr),   all_times = [all_times; gsr.pc_time];   end
    if ~isempty(accel), all_times = [all_times; accel.pc_time]; end
    if ~isempty(force), all_times = [all_times; force.pc_time]; end
    t0 = min(all_times);
    if ~isempty(gsr),   gsr.t   = gsr.pc_time   - t0; end
    if ~isempty(accel), accel.t = accel.pc_time  - t0; end
    if ~isempty(force), force.t = force.pc_time  - t0; end
end

% function [sig_ds, t_ds] = downsample_block(sig, t, factor)
%     n = length(sig); n_ds = floor(n/factor);
%     sig_ds = zeros(n_ds,1); t_ds = zeros(n_ds,1);
%     for k = 1:n_ds
%         idx = (k-1)*factor+1 : k*factor;
%         sig_ds(k) = mean(sig(idx)); t_ds(k) = t(idx(1));
%     end
% end
% New function — replace the old downsample_block() entirely:
function [sig_ds, t_ds] = antialias_downsample(sig, t, fs_in, fs_out, order, ds_factor)
    % Zero-phase Butterworth LPF at 90% of new Nyquist, THEN block-average.
    % filtfilt() = no time shift of transient events.
    cutoff_hz = fs_out / 2 * 0.9;          % guard band below new Nyquist
    Wn        = cutoff_hz / (fs_in / 2);   % normalise to [0,1] for butter()
    Wn        = min(Wn, 0.99);             % must be strictly < 1
    [b, a]    = butter(order, Wn, 'low');
    sig_filt  = filtfilt(b, a, double(sig));  % zero-phase forward+backward
...
function [merged_times, merged_source] = merge_close_events_tagged(tagged, window)
    % tagged: Nx2 [timestamp, source].  Events within window are merged.
    % Sources: 1=accel, 2=force -> if both hit same event, becomes 3=both.
...
function [scr_thresh, scl_thresh] = compute_gsr_thresholds_cvx(p_cvx, t_cvx, scr_sens, scl_sens)
    % WHY: p_cvx is z-scored µS — its scale changes every rep.
    % We threshold relative to the noise floor of the phasic component itself.
    abs_p      = abs(p_cvx);
    scr_thresh = median(abs_p) + scr_sens * mad(abs_p, 1);
    scr_thresh = max(scr_thresh, 1e-4);   % hard floor

    % SCL: threshold changes in tonic level (diff removes slow absolute drift)
...
function [scr_thresh, scl_thresh] = compute_gsr_thresholds_raw(gsr_raw, scr_sens, scl_sens)
    % WHY: raw resistance varies from ~1 kΩ to >1 MΩ — a fixed 0.05 Ohm
    % threshold is effectively zero and will flag everything.
    delta_fast = abs(diff(gsr_raw));
    scr_thresh = median(delta_fast) + scr_sens * mad(delta_fast, 1);
    scr_thresh = max(scr_thresh, 1.0);   % hard floor: 1 Ohm minimum

    win_pts    = max(3, round(length(gsr_raw) * 0.01));   % 1% of signal
