...
% -----------------------------------------------------------------------
base_folder   = '/home/acatalano/VIRTUES/recordings/subject_accFix/Baseline';
...
for lv = levels_to_run
    ...
    %% -------------------------------------------------------------------
    % 2. BUILD SENSOR TABLES
    % -------------------------------------------------------------------
    % Accelerometer channels (ai1-ai6)
    accel_raw          = table();
    ...
    force_raw          = table();
    force_raw.F1       = nidaq.ai7  - nidaq.ai8;
    force_raw.F2       = nidaq.ai9  - nidaq.ai10;
    force_raw.F3       = nidaq.ai11 - nidaq.ai12;
    force_raw.F4       = nidaq.ai13 - nidaq.ai14;
    force_raw.F5       = nidaq.ai15 - nidaq.ai16;
    force_raw.F6       = nidaq.ai17 - nidaq.ai18;
    force_raw.pc_time  = nidaq.pc_time;

    %% -------------------------------------------------------------------
    % 3. RECONSTRUCT UNIFORM TIMESTAMPS (block-aware)
    % -------------------------------------------------------------------
    ...
    %% -------------------------------------------------------------------
    % 4b. BASELINE OFFSET REMOVAL + CONVERT V -> G
    % Subtract the mean of the first 100 samples from every channel
    % (same approach as: sig_ori = sig - mean(sig(1:100,:)) ).
    % This zeros the sensor activation offset without discarding any data.
    % Accel conversion: sensitivity = 0.4 V/g  =>  g = V / 0.4
    % -------------------------------------------------------------------
    n_baseline = 100;   % samples used to estimate the resting offset
    ...
    for k = 1:numel(force_cols)
        c = force_cols{k};
        force_raw.([c '_smooth']) = movmean(force_raw.(c), smooth_window);
    end
    force_raw.mag = sqrt(force_raw.F1_smooth.^2 + force_raw.F2_smooth.^2 + ...
                         force_raw.F3_smooth.^2 + force_raw.F4_smooth.^2 + ...
                         force_raw.F5_smooth.^2 + force_raw.F6_smooth.^2);

    %% -------------------------------------------------------------------
    % 6. COLLISION DETECTION (on downsampled + filtered signal)
    % -------------------------------------------------------------------
    % Detection runs on the DOWNSAMPLED signal, not raw 3000 Hz.
    % abs(diff()) on raw data turns sample-to-sample noise into thousands
    % of false threshold crossings. After anti-alias filtering and
    % downsampling to target_fs_display the derivative is clean.
