...
SAVE_PATH   = '/home/acatalano/Desktop/Virtues/EDA_Results';
GSR_UNIT    = 'ohm';
MA_WIN      = 1.0;
USE_CVXEDA  = true;
K_MAD=2.5; MIN_DIST=1.0; MIN_AMP=0.01;

if ~isfolder(SAVE_PATH), mkdir(SAVE_PATH); end

subj_folders = scan_subjects(BASE_FOLDER);
...
for s = 1:numel(subj_folders)
    subj_name  = subj_folders{s};
    subj_short = subj_name(9:end);
    subj_mod   = subj_short(end);

    fprintf('%s  TEST\n', subj_short);

    rest_path = fullfile(BASE_FOLDER, subj_name, 'resting_state', [subj_short '_r1']);
    [bsl_mean, bsl_std] = get_rest_baseline(rest_path, GSR_UNIT, MA_WIN);

    for a = 1:2
        test_path = fullfile(BASE_FOLDER, subj_name, sprintf('Test%d', a));
        if ~isfolder(test_path), continue; end

        [t_gsr, gsr_vals] = load_gsr(test_path);
        if isempty(t_gsr), continue; end

        [t_eda, eda_uS, fs] = prepare_eda(t_gsr, gsr_vals, GSR_UNIT, MA_WIN);
        if isempty(t_eda), continue; end

        if isfinite(bsl_mean) && bsl_std > 0
            eda_z = (eda_uS - bsl_mean) / bsl_std;
        else
            eda_z = robust_detrend(eda_uS);
        end

        [phasic, tonic, method] = decompose_eda(eda_z, fs, USE_CVXEDA);
        fprintf('  acq%d: fs=%.1fHz  dur=%.0fs  %s\n', a, fs, t_eda(end), method);

        tf = trial_features(t_eda, tonic, phasic, fs, K_MAD, MIN_DIST, MIN_AMP);

        ri = ri+1;
        results(ri).subject_id  = subj_short;
        results(ri).subj_mod    = subj_mod;
        results(ri).phase       = 'TEST';
        results(ri).acq         = a;
        results(ri).level       = NaN;
        results(ri).method      = method;
        results(ri).bsl_mean_uS = bsl_mean;
        results(ri).bsl_std_uS  = bsl_std;
        fields = fieldnames(tf);
        for fi=1:numel(fields), results(ri).(fields{fi}) = tf.(fields{fi}); end

        fig=figure('Name',sprintf('TEST %s a%d',subj_short,a), ...
            'Position',[30 30 1400 400],'Visible','off');
        sgtitle(sprintf('TEST  |  %s  |  Acq%d  |  %s',subj_short,a,method), ...
            'FontWeight','bold','FontSize',11);
        subplot(1,1,1);
        plot_eda_panel(t_eda,eda_uS,tonic,phasic, ...
            sprintf('%s Test%d  %.0fs',subj_short,a,t_eda(end)));
        saveas(fig,fullfile(SAVE_PATH,sprintf('TEST_%s_a%d.png',subj_short,a)));
        close(fig);
    end
end

T = struct2table(results);
save(fullfile(SAVE_PATH,'EDA_FEATURES_TEST.mat'),'T','-v7.3');
fprintf('\nTEST done — %d rows\n', height(T));

plot_group_summary(T,'TEST',SAVE_PATH);
...
function [bsl_mean, bsl_std] = get_rest_baseline(rest_path, gsr_unit, ma_win)
    bsl_mean=NaN; bsl_std=NaN;
    if ~isfolder(rest_path), return; end
    [t_gsr,gsr_vals]=load_gsr(rest_path);
    if isempty(t_gsr), return; end
    [~,eda_uS,~]=prepare_eda(t_gsr,gsr_vals,gsr_unit,ma_win);
    if isempty(eda_uS), return; end
    bsl_mean=mean(eda_uS,'omitnan');
    bsl_std=std(eda_uS,0,'omitnan');
end