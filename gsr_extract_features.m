...
%   - Baseline/Test acquisitions ('Level1'..'Level5') are each single, unrepeated trials, so 'repetition' is NaN for those rows.
    ...
    for si = 1:numel(cfg.all_subjects)
        ...
        for ph_idx = 1:numel(all_phases)
            ...
            for ai = 1:numel(acquisitions)
                ...
                % Within-trial linear trend (uS/s): arousal build-up/decay over the course of a single trial, distinct from the
                % across-repetition trajectory captured by the LMM's per-subject random slope on `repetition`.
                pp_trial  = polyfit(t_gsr, tonic, 1);
                scl_slope = pp_trial(1);

                % --- Baseline-corrected SCL/GSR (delta and % change from
                % resting reference). Only tonic-level measures are baseline-corrected here: SCR amplitude/frequency during
                % quiet rest is near-zero/undefined, so subtracting a resting phasic value is not standard practice (Boucsein,
                ...
                rows(end+1, :) = {subj, group, haptic, phase_type, phase, acq, ...
                    level_this, rep_this, duration_s, scl_mean, scl_range, scl_slope, scr_freq, scr_mean_amp, scr_max_amp, scr_auc, ...
                    gsr_mean_, gsr_std_, gsr_rms_, scl_mean_bc, scl_mean_pctchg, gsr_mean_bc, gsr_mean_pctchg, ...
    ...
    varnames = {'subject','group','haptic','phase_type','phase','acquisition', ...
        'level','repetition','duration_s',  'scl_mean','scl_range','scl_slope','scr_freq','scr_mean_amp','scr_max_amp','scr_auc', ...
        'gsr_mean','gsr_std','gsr_rms', 'scl_mean_bc','scl_mean_pctchg','gsr_mean_bc','gsr_mean_pctchg', ...
