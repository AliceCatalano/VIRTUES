...
class ShimmerNode(Node):
    ...
    def connect_rfcomm(self, mac="00:06:66:D7:C9:C7", channel="1", dev="/dev/rfcomm0",
                       wait_secs=5, release_idx=0):
        """Start rfcomm connect and wait until /dev/rfcomm0 exists and is openable.
    ...
    def initialize_shimmer(self):
        ...
        self.get_logger().info("initialize function")
        
        # First attempt to connect rfcomm
        if not self.connect_rfcomm():
            self.get_logger().warn("rfcomm connect failed on first attempt — node will keep retrying.")
            
            # Retry loop - CHECK if context is initialized before continuing
            while rclpy.ok():
                if self.connect_rfcomm():
                    break
                time.sleep(2.0)
        
        # Now try to open the serial port with retries
        # CRITICAL FIX: Use /dev/rfcomm0 not /dev/rfcomm1
        for attempt in range(6):
            try:
                self.serial = serial.Serial("/dev/rfcomm0", 115200, timeout=1)
                self.get_logger().info("Port opened successfully")
                time.sleep(0.5)  # Let port stabilize
                break
            except serial.SerialException as e:
                self.get_logger().warn(f"Opening /dev/rfcomm0 attempt {attempt+1} failed: {e}")
                time.sleep(1.0)
        else:
            self.get_logger().error("Failed to open /dev/rfcomm0 after retries. Exiting.")
            return
