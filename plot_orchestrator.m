...
% Configuration
% ------------------------------------------------------------------------
session_folder = '/home/acatalano/VIRTUES/recordings/subject_io_Pink_feedback/Baseline/Level1';
...
force = table();
force.F1 = nidaq.ai7  - nidaq.ai8;
force.F2 = nidaq.ai9  - nidaq.ai10;
force.F3 = nidaq.ai11 - nidaq.ai12;
force.F4 = nidaq.ai13 - nidaq.ai14;
force.F5 = nidaq.ai15 - nidaq.ai16;
force.F6 = nidaq.ai17 - nidaq.ai18;
...
%% ------------------------------------------------------------------------
% Force processing (moving average)
% ------------------------------------------------------------------------
force_cols = {'F1','F2','F3','F4','F5','F6'};
for k = 1:numel(force_cols)
    c = force_cols{k};
    force.([c '_smooth']) = movmean(force.(c), smooth_window);
end
...
for k = 1:numel(accel_cols)
    c = accel_cols{k};
    accel.([c '_smooth']) = movmean(accel.(c), smooth_window);
end
...
% Smooth per accelerometer
smooth_window = accel_fs/10;
accel.mag_L_smooth = movmean(accel.mag_L, smooth_window);
accel.mag_R_smooth = movmean(accel.mag_R, smooth_window);
...
% title(sprintf('Pupil Diameter, rep %d', rep));
% legend('Left','Right');
% grid on;
% xline(event_times,'k');
%% ------------------------------------------------------------------------
...
% Accelerometers
figure;
plot(accel.t, accel.mag_L_smooth, 'r', accel.t, accel.mag_R_smooth, 'b', 'LineWidth', 1.5);
ylabel('Accel magnitude');
legend('Left','Right');
title(sprintf('Accelerometer Magnitude (smoothed), rep %d', rep));
grid on;
xline(event_times,'k');
...
figure; hold on;
% for k = 1:numel(force_cols)
%     plot(force.t, force.([force_cols{k} '_smooth']));
% end

forcemag = sqrt(force.([force_cols{1} '_smooth']).^2+force.([force_cols{5} '_smooth']).^2+force.([force_cols{6} '_smooth']).^2);
plot(force.t,forcemag)
ylabel('Force [V]');
title(sprintf('Force Sensors (smoothed), rep %d', rep));
grid on;
xline(event_times,'k');
legend(force_cols);
%% ========================================================================
