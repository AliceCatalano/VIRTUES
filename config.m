...
's23H','s25H','s26H','s29H','s31H','s33H','s35H','s38H','s40H','s41H', 's45H','s47H'};  %{'s44N','s46N','s45H','s47H'}
    ...
    cfg.scr_min_dist     = 10;
    % SCR noise-floor threshold: noise_floor = median(|phasic|) + scr_sensitivity * MAD(phasic)
    % Lower = more sensitive (catches smaller SCRs, more false positives).
    % Higher = stricter. 2.5 is a reasonable starting point; tune against
    % a few subjects' plots in gsr_collision_overview.m before trusting detection counts at scale.
    cfg.scr_sensitivity  = 2.5;
    cfg.max_gap_s        = 0.5;
