...
def convert_bag(bag_dir: str, out_path: str) -> str:
    ...
    if frame_count == 0:
        print(f"  [ERROR] No frames decoded — check IMAGE_TOPIC.")
...
def main():
    # Find all video_bag directories under level_L* / rep_*
    # OLD structure: level_L* / rep_* / video_bag
    # pattern = os.path.join(BASE_DIR, "level_L*", "rep_*", "video_bag")
    # pattern = os.path.join(BASE_DIR, "Level*",  "video_bag")
    ...
    for bag_dir in bag_dirs:
        # OLD: extract rep number from folder name (e.g. rep_03 → 3)
        # rep_folder = os.path.basename(os.path.dirname(bag_dir))  # e.g. "rep_03"
        # level_folder = os.path.basename(os.path.dirname(os.path.dirname(bag_dir)))  # e.g. "level_L2"
        # level_num    = level_folder.split("l")[-1]   # e.g. "L2"
        # rep_num    = rep_folder.split("_")[-1].lstrip("0") or "0"  # e.g. "3"
        # out_name     = f"video_{level_num}_{rep_folder}.mp4"  # e.g. "video_L2_rep_03.mp4"

        # NEW: only Level# is changing, no rep subfolder
        level_folder = os.path.basename(os.path.dirname(bag_dir))  # e.g. "Level1"
        level_num    = level_folder.replace("Level", "")            # e.g. "1"
        out_name     = f"video_Level{level_num}.mp4"                # e.g. "video_Level1.mp4"
